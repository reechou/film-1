...
(function(seajs, global) {
  ...
  function xhr(url, callback) {
    ...
    var isAndroid = global.navigator.userAgent.toLowerCase().indexOf("android") > -1
    var appCached = global.applicationCache && global.applicationCache.status !== 0
    r.onreadystatechange = function() {
      if (r.readyState === 4) {
        if (r.status === 200 || (isAndroid && appCached && r.status === 0)) {
  ...
  function jsEscape(content) {
    return content.replace(/(["\\])/g, '\\$1').replace(/[\t\n\r]/g, '');
  }
