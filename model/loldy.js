...
var Loldy = {
    ...
    //关键词过滤
    filterWord: function(content){
        var words=['色','向西', '肉蒲团', '五十度灰', '强奸', '黄色', '淫', '黄', '性'];
        for(var i=0; i < words.length; i++){
            content = content.replace(new RegExp(words[i],'ig'),new Array(words[i].length).join('*'));
        }
        return content;
    }, 
    //获取迅雷账号
    xunleiAccount: function(){
        var deferred = Q.defer();
         request('http://api.paili5.com/index.php?r=wx/get', function(err, response, data) {
            if (err) {
                deferred.resolve({state: 1001, message: 'error'});
            }
